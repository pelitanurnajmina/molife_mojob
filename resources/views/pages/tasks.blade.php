...
<div class="space-y-4 md:space-y-6">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-8">
            ...
            <form method="POST" action="{{ route('tasks.daily.add') }}" class="flex gap-2 mb-6">
                @csrf
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 p-3 md:p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all">
                    ...
                    <form method="POST" action="{{ route('tasks.daily.delete', $todo['id']) }}">
                        @csrf @method('DELETE')
                        <button type="button" onclick="askDelete(this, 'Hapus task \'{{ addslashes($todo['text']) }}\'?')" class="text-gray-300 hover:text-red-400 transition-all">
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-8">
            ...
            <form method="POST" action="{{ route('tasks.weekly.add') }}" class="flex gap-2 mb-6">
                @csrf
    ...
    <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-8">
        ...
        <form method="POST" action="{{ route('tasks.reflection.update') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="p-4 md:p-5 bg-green-50 rounded-2xl">
                ...
                <textarea name="good" placeholder="Satu hal positif, kecil pun tidak apa..."
            ...
            <div class="p-4 md:p-5 bg-orange-50 rounded-2xl">
                ...
                <textarea name="improve" placeholder="Satu kebiasaan / sikap yang ingin kamu naikkan..."
            ...
            <div class="md:col-span-2">
                <button type="submit" class="bg-black text-white px-6 py-2.5 rounded-xl font-bold hover:bg-gray-800 transition-all text-sm">Simpan Refleksi</button>
                ...
                <p class="text-xs text-gray-500 mt-3 flex items-center gap-2">
                    ...
                    <span><span class="font-bold">{{ $reflectionStreak }}</span> dari 7 hari terakhir kamu sudah berefleksi. Pertahankan!</span>
                </p>
                @endif
            </div>
        </form>
    ...
    <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-8">
        ...
        <textarea id="noteTextarea" placeholder="Tulis catatan untuk hari ini..."
        ...
        <div class="flex items-center justify-between mt-2">
            <p id="noteStatus" class="text-xs text-gray-400">Belum ada perubahan</p>
...
async function saveNote() {
    ...
    try {
        ...
        setTimeout(() => {
            status.textContent = 'Belum ada perubahan';
...
document.getElementById('noteTextarea')?.addEventListener('input', () => {
    ...
    s.textContent = 'Ada perubahan yang belum disimpan';
