...
<div class="w-full max-w-lg">
    ...
    <form method="POST" action="{{ route('onboarding.store') }}" id="onboardingForm">
        ...
        <div class="step-panel active bg-white rounded-3xl p-6 md:p-8 shadow-sm" id="step1">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{{ __('Langkah :n dari 3', ['n' => 1]) }}</p>
            <h2 class="text-2xl font-bold mb-1">{{ __('Selamat datang di Molife') }} 👋</h2>
            <p class="text-sm text-gray-500 mb-6">{{ __('Yuk kenalan dulu, siapa namamu?') }}</p>

            <input type="text" name="display_name" id="displayName"
                autocomplete="off"
                placeholder="{{ __('Nama atau panggilanmu...') }}"
            ...
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-6 mb-2">{{ __('Jenis Kelamin') }}</p>
            <div class="grid grid-cols-2 gap-2.5">
                <div class="gender-card flex items-center justify-center gap-2 p-4 rounded-2xl border-2 border-gray-100 bg-gray-50 hover:border-gray-300" onclick="selectGender('male', this)">
                    ...
                    <span class="font-bold text-sm">{{ __('Laki-laki') }}</span>
                </div>
                <div class="gender-card flex items-center justify-center gap-2 p-4 rounded-2xl border-2 border-gray-100 bg-gray-50 hover:border-gray-300" onclick="selectGender('female', this)">
                    ...
                    <span class="font-bold text-sm">{{ __('Perempuan') }}</span>
                </div>
            </div>
            <p class="text-[11px] text-gray-400 mt-2">{{ __('Dipakai untuk menyesuaikan fitur (mis. penanganan hari uzur di tracker sholat).') }}</p>
        ...
        <div class="step-panel bg-white rounded-3xl p-6 md:p-8 shadow-sm" id="step2">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{{ __('Langkah :n dari 3', ['n' => 2]) }}</p>
            <h2 class="text-2xl font-bold mb-1">{{ __('Olahraga apa saja?') }}</h2>
            <p class="text-sm text-gray-500 mb-6">{{ __('Pilih yang kamu lakukan. Bisa lebih dari satu, atau lewati jika tidak ada.') }}</p>

            @php
            $sportOptions = [
                ['value'=>'gym',         'label'=>__('Gym / Fitness'),       'sub'=>__('Latihan beban, kardio'),    'icon'=>'M13 10V3L4 14h7v7l9-11h-7z'],
                ['value'=>'run',         'label'=>__('Lari / Running'),      'sub'=>__('Outdoor, treadmill'),       'icon'=>'M22 12h-4l-3 9L9 3l-3 9H2'],
                ['value'=>'cycling',     'label'=>__('Bersepeda'),           'sub'=>__('Road bike, MTB, sepeda santai'), 'icon'=>'M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4'],
                ['value'=>'swimming',    'label'=>__('Renang'),              'sub'=>__('Kolam renang, open water'), 'icon'=>'M7 16.5c2 1 4 1 6 0s4-1 6 0M7 11.5c2 1 4 1 6 0s4-1 6 0M3 7.5c2 1 4 1 6 0s4-1 6 0m-9 11V7.5'],
                ['value'=>'racket',      'label'=>__('Tenis / Badminton'),   'sub'=>__('Padel, squash, dan lainnya'),'icon'=>'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z'],
                ['value'=>'custom_sport','label'=>__('Olahraga Lainnya'),   'sub'=>__('Voli, basket, sepak bola, dll.'), 'icon'=>'M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z'],
            ];
            @endphp
            ...
            <div id="customSportInput" class="hidden mb-4">
                <input type="text" id="customSportName" placeholder="{{ __('Nama olahraga kamu (mis: Voli, Padel...)') }}"
            ...
            <div class="flex gap-3 mt-2">
                <button type="button" onclick="goStep(1)" class="flex-1 py-4 bg-gray-100 rounded-2xl font-bold text-sm hover:bg-gray-200 transition-all">{{ __('Kembali') }}</button>
                <button type="button" onclick="goStep(3)" class="flex-[2] py-4 bg-black text-white rounded-2xl font-bold text-sm hover:bg-gray-800 transition-all">{{ __('Lanjut') }}</button>
            </div>
        </div>

        {{-- ── Step 3: Other features ── --}}
        <div class="step-panel bg-white rounded-3xl p-6 md:p-8 shadow-sm" id="step3">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{{ __('Langkah :n dari 3', ['n' => 3]) }}</p>
            <h2 class="text-2xl font-bold mb-1">{{ __('Fitur yang mau kamu pakai') }}</h2>
            <p class="text-sm text-gray-500 mb-6">{{ __('Aktifkan yang relevan. Semua bisa diubah lagi di Pengaturan.') }}</p>

            @php
            $featOptions = [
                ['value'=>'timeblock','label'=>__('Time Blocking'), 'sub'=>__('Jadwalkan harimu per blok waktu'), 'on'=>true,  'icon'=>'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
                ['value'=>'tasks',    'label'=>__('Tasks & Notes'), 'sub'=>__('To-do harian & catatan'),        'on'=>true,  'icon'=>'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4'],
                ['value'=>'pomodoro', 'label'=>__('Pomodoro'),      'sub'=>__('Timer fokus produktif'),          'on'=>true,  'icon'=>'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
                ['value'=>'meditasi', 'label'=>__('Meditasi'),      'sub'=>__('Timer meditasi & napas'),         'on'=>true,  'icon'=>'M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z'],
                ['value'=>'mental',   'label'=>__('Mental'),        'sub'=>__('Mood & refleksi harian'),         'on'=>true,  'icon'=>'M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                ['value'=>'journal',  'label'=>__('Journal'),       'sub'=>__('Journaling terpandu'),            'on'=>true,  'icon'=>'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z'],
                ['value'=>'ide_script','label'=>__('Ide & Script'), 'sub'=>__('Kumpulan ide & script konten'),   'on'=>true,  'icon'=>'M9.663 17h4.673M12 3a6 6 0 00-3.6 10.8c.44.33.6.87.6 1.42V16a1 1 0 001 1h4a1 1 0 001-1v-.78c0-.55.16-1.09.6-1.42A6 6 0 0012 3z'],
                ['value'=>'motivasi', 'label'=>__('Motivasi'),      'sub'=>__('Quote & dampak konsistensi'),     'on'=>true,  'icon'=>'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 3v-3z'],
                ['value'=>'goals',    'label'=>__('Goals & Reminder'),'sub'=>__('Target bulanan & pengingat'),   'on'=>true,  'icon'=>'M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z'],
                ['value'=>'links',    'label'=>__('Link Penting'),  'sub'=>__('Simpan link penting'),            'on'=>true,  'icon'=>'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1'],
                ['value'=>'finance',  'label'=>__('Finance'),       'sub'=>__('Transaksi, anggaran, tabungan'),  'on'=>true,  'icon'=>'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 9v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                ['value'=>'lamaran',  'label'=>__('Karir & Lamaran'),'sub'=>__('Lacak lamaran kerja'),           'on'=>true,  'icon'=>'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
                ['value'=>'bisnis',   'label'=>__('Bisnis'),        'sub'=>__('Proposal, klien & analitik'),     'on'=>true,  'icon'=>'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6'],
                ['value'=>'intimasi', 'label'=>__('Intimasi'),      'sub'=>__('Tracker bersama pasangan'),       'on'=>false, 'icon'=>'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z'],
                ['value'=>'porn',     'label'=>__('Stop Porn'),     'sub'=>__('Streak bebas pornografi'),        'on'=>false, 'icon'=>'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z'],
                ['value'=>'sosmed',   'label'=>__('Kurangi Sosmed'),'sub'=>__('Disiplin waktu media sosial'),    'on'=>false, 'icon'=>'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z'],
            ];
            @endphp
            ...
            <div class="flex gap-3 mt-5">
                <button type="button" onclick="goStep(2)" class="flex-1 py-4 bg-gray-100 rounded-2xl font-bold text-sm hover:bg-gray-200 transition-all">{{ __('Kembali') }}</button>
            ...
            <p class="text-center text-xs text-gray-400 mt-3">{{ __('Bisa diubah kapan saja di Pengaturan.') }}</p>
        </div>
