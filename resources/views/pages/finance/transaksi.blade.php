    ...
    <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-8">
        ...
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all group">
                ...
                <form method="POST" action="{{ route('finance.transaksi.delete', $tx['id']) }}" class="opacity-0 group-hover:opacity-100 transition-all">
                    @csrf @method('DELETE')
                    <button type="submit" onclick="return confirm('{{ __('Hapus transaksi ini?') }}')"
